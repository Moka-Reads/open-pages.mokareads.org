...
<html lang="en">
    <head>
        ...
        <!-- Cloudflare Script Exclusion -->
        <script data-cfasync="false" data-cf-settings="rocket=0">
            // Disable Cloudflare optimizations globally
    ...
    <body class="loading">
        ...
        <!-- WASM Papers Manager - Non-Module Fallback -->
        <script data-cfasync="false" data-cf-settings="rocket=0">
            ...
            async function loadWasmNonModule() {
                try {
                    ...
                    moduleScript.setAttribute('data-cf-settings', 'rocket=0');

                    const scriptContent = `
                        (async () => {
                            try {
                                const wasmModule = await import('./pkg/open_pages_processor.js');
                                window.wasmModuleLoaded = wasmModule;
                                window.dispatchEvent(new CustomEvent('wasmModuleReady'));
                            } catch (error) {
                                window.wasmModuleError = error;
                                window.dispatchEvent(new CustomEvent('wasmModuleError'));
                            }
                        })();
                    `;

                    moduleScript.textContent = scriptContent;

                    // Wait for module to load
                    const modulePromise = new Promise((resolve, reject) => {
                        window.addEventListener('wasmModuleReady', () => {
                            resolve(window.wasmModuleLoaded);
                        }, { once: true });

                        window.addEventListener('wasmModuleError', () => {
                            reject(window.wasmModuleError);
                        }, { once: true });
